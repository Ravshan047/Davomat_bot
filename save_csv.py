...
async def day_attendance(data):
    ...
    if file_exists:
        async with aiofiles.open(f"{today}.csv", mode="r", encoding="utf-8") as file:
            contents = await file.read()  # Faylning mazmunini asinxon o'qish
            reader = csv.reader(contents.splitlines())
            next(reader)  # Sarlavhani o'tkazib yubormaymiz

            # Telegram ID faylda bor yoki yo'qligini tekshiramiz
            for row in reader:
                if len(row) > 1 and row[1].strip() == str(telegram_id):  # Telegram ID ni tekshirish
                    return False  # Foydalanuvchi topildi, yozmaymiz
    ...
    async with aiofiles.open(f"{today}.csv", mode="a", newline='', encoding="utf-8") as file:
        ...
        if not file_exists:
            ...
            # Foldalanuvchi malumotlarini yozish
