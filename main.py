...
from kalendat import create_calendar
from save_csv import save_to_csv, get_user_buttons, day_attendance

API_TOKEN = '7537380960:AAFRYKD0LiXrOStzKIOmqG86e63sH0rViac'
...
async def process_day_selection(callback_query: CallbackQuery):
    ...
    try:
        if callback_query. from_user.id == 104745314:
            ...
            print("jjjjjj")
            await bot. answer_callback_query(callback_query.id, f"Kalendar orgali faqat admin ko'ra oladi! !! ",
                                             show_alert=True)
                ...
                for row in reader:
                    if row[1] == str(telegram_id) and row[0] == name:  # Telegram ID va ismi tekshirish
                        ...
                        if await day_attendance([name, telegram_id, 'ha']) is False:
                            print("l")
                            await bot.answer_callback_query(callback_query.id, 'Siz bugun uchun davomat topshirdingiz!',
                                                            show_alert=True)
                        else:
...
async def start_bot(bot: Bot):
    await bot.send_message(6694931501, "Bot ishga tushdi !! ")

async def shutdown_bot(bot: Bot):
    await bot.send_message(6694931501, "Bot o'chdi !! ")
